              ...
              {coursesOpen && (
                <div className="absolute top-full left-0 pt-2 animate-fade-in">
                  <div className="bg-white border border-border rounded-xl shadow-lg py-2 min-w-[180px]">
                    {categories.map((cat) => (
                      <Link key={cat.slug} href={`/kurs-kategori/${cat.slug}`} className="block px-4 py-2 text-[13px] text-gray-dark hover:bg-bg hover:text-purple transition-colors">
                        {cat.name}
                      </Link>
                    ))}
                  </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-border animate-fade-in">
          <div className="px-4 py-3 space-y-1">
            ...
            {categories.map((cat) => (
              <Link key={cat.slug} href={`/kurs-kategori/${cat.slug}`} onClick={() => setMobileOpen(false)} className="block px-6 py-2 text-[13px] text-gray hover:bg-bg rounded-lg">
                {cat.name}
              </Link>
            ))}
